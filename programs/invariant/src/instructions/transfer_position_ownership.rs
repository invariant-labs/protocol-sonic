...
// Pool addresses that allows user to earn points
const DISABLED_POOLS: [&'static str; 1] = [
    "GxUVSXpHSzHahyXDrSDAc6C5DzC1KPG4E5o9kjJFUWKQ", // Pool address used in tests
];
...
impl<'info> TransferPositionOwnership<'info> {
    pub fn handler(&self, index: u32, bump: u8) -> Result<()> {
        ...
        let disabled_list: Vec<&str> = DISABLED_POOLS.iter().copied().collect();

        if disabled_list.contains(&removed_position.pool.key().to_string().as_str()) {
            return Err(ErrorCode::DisabledPool.into());
        }
