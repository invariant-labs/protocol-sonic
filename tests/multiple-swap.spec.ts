...
describe('Multiple swap', () => {
  ...
  it('swap X to Y 10 times', async () => {
    ...
    const initPositionVars: CreatePosition = {
      ...
      upperTick,
      liquidityDelta,
      knownPrice: PRICE_DENOMINATOR, // initial price = 1.0
      slippage: new BN(0) // 0% slippage
    }
    await market.createPosition(initPositionVars, user1)
      ...
      await market.swap(
        {
          pair,
          amount: swapAmount,
          estimatedPriceAfterSwap: priceAfterSwap,
    ...
    assert.ok(feeGrowthGlobalX.eqn(0))
    assert.ok(feeGrowthGlobalY.eqn(0))
    assert.ok(feeProtocolTokenX.eqn(10))
    assert.ok(feeProtocolTokenY.eqn(0))
    assert.ok(liquidity.eq(liquidityDelta))
    assert.ok(sqrtPrice.eq(new BN('959803483698079499776690')))
  ...
  it.only('swap Y to X 10 times', async () => {
    // initialize market
    const tokens = await Promise.all([
      createToken(connection, wallet, mintAuthority),
      createToken(connection, wallet, mintAuthority)
    ...
    assert.ok(liquidity.eq(liquidityDelta))
    assert.ok(sqrtPrice.eq(new BN('1041879944160074453234060')))
