...
describe('withdraw', () => {
  ...
  it('#create position', async () => {
    ...
    const mintAmount = BigInt(new BN(10).pow(new BN(10)).toString())
    await mintTo(
      connection,
      mintAuthority,
      pair.tokenX,
    ...
    liquidityDelta = new BN(1000000).mul(LIQUIDITY_DENOMINATOR)
  ...
  it('#withdraw with incorrect ticks should failed', async () => {
    ...
    const initPositionVars: CreatePosition = {
      ...
      userTokenY: userTokenYAccount,
      owner: positionOwner.publicKey,
      liquidityDelta: liquidityDelta.muln(1_000_000),
      pair: pair,
      slippage: new BN(0)
    }
    await market.createPosition(initPositionVars, positionOwner)
    ...
    const removePositionIx = await market.program.methods
      ...
      .accounts({
        ...
        eventOptAcc: market.getEventOptAccount(pair.getAddress(market.program.programId)).address
      })
      .instruction()
  ...
  it('#withdraw', async () => {
    ...
    assert.ok(poolData.feeGrowthGlobalX.eq(new BN('4999995000004')))
