...
describe('cross', () => {
  ...
  it('#swap() with crossing a tick', async () => {
    ...
    const initPositionVars1: CreatePosition = {
      pair,
    ...
    await market.createPosition(initPositionVars1, positionOwner)
    ...
    assert.ok(poolData.feeGrowthGlobalX.eq(new BN('4000000000000000000'))) // 0.6 % of amount - protocol fee
    ...
    assert.ok(upperTickData.feeGrowthOutsideX.eqn(0))
    assert.ok(middleTickData.feeGrowthOutsideX.eq(new BN('3000000000000000000')))
    assert.ok(lowerTickData.feeGrowthOutsideX.eqn(0))
  })
})
