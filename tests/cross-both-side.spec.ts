...
describe('swap with cross both side', () => {
  ...
  it('push price to tick without crossing and push price to tick with crossing', async () => {
    ...
    assert.ok(poolData.sqrtPrice.eq(sqrtPriceAtTicks))
    ...
    const massiveCreatePositionMassive: CreatePosition = {
      pair,
      owner: positionOwner.publicKey,
      userTokenX: userTokenXAccount,
      userTokenY: userTokenYAccount,
    ...
    await market.createPosition(massiveCreatePositionMassive, positionOwner)
    ...
    assert.ok(finalPool.feeGrowthGlobalX.eq(new BN('2999100269919024292')))
    assert.ok(finalPool.feeGrowthGlobalY.eq(new BN(0)))
    assert.ok(finalPool.feeProtocolTokenX.eq(new BN(4)))
    assert.ok(finalPool.feeProtocolTokenY.eq(new BN(2)))
    assert.ok(finalPool.liquidity.eq(new BN('19996000399699901991603000000')))
    assert.ok(finalPool.sqrtPrice.eq(new BN('999500149964999999999999')))

    const finalLastTick = await market.getTick(pair, lastTick)
    assert.ok(finalLastTick.feeGrowthOutsideX.eq(new BN(0)))
    assert.ok(finalLastTick.feeGrowthOutsideY.eq(new BN(0)))
    assert.ok(finalLastTick.liquidityChange.eq(new BN('19996000399699901991603000000')))

    const finalLowerTick = await market.getTick(pair, lowerTick)
    assert.ok(finalLowerTick.feeGrowthOutsideX.eq(new BN('2999100269919024292')))
    assert.ok(finalLowerTick.feeGrowthOutsideY.eq(new BN(0)))
    assert.ok(finalLowerTick.liquidityChange.eq(new BN(0)))

    const finalUpperTick = await market.getTick(pair, upperTick)
    assert.ok(finalUpperTick.feeGrowthOutsideX.eq(new BN(0)))
    assert.ok(finalUpperTick.feeGrowthOutsideY.eq(new BN(0)))
    assert.ok(finalUpperTick.liquidityChange.eq(new BN('20006000000000')))
  })
})
