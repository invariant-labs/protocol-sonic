...
import { Tick, Tickmap } from '@invariant-labs/sdk/src/market'
...
import { parseTick } from '@invariant-labs/sdk/lib/market'
...
describe('Math', () => {
  ...
  describe('Test sqrt price calculation', () => {
    it('Test 20000', () => {
      ...
      assert.ok(result.eq(new BN('2718145925979' + '0'.repeat(PRICE_SCALE - 12))))
    })
    it('Test 200000', () => {
      const price = 200000
      const result = calculatePriceSqrt(price)
      // expected 22015.455979766288
      assert.ok(result.eq(new BN('22015455979766288' + '0'.repeat(PRICE_SCALE - 12))))
    })
    it('Test -20000', () => {
      const price = -20000
      const result = calculatePriceSqrt(price)
      // expected 0.367897834491
      assert.ok(result.eq(new BN('367897834491' + '0'.repeat(PRICE_SCALE - 12))))
    })
    it('Test -200000', () => {
      const price = -200000
      const result = calculatePriceSqrt(price)
      // expected 0.000045422634
      assert.ok(result.eq(new BN('45422634' + '0'.repeat(PRICE_SCALE - 12))))
    })
    it('Test 0', () => {
      const price = 0
      const result = calculatePriceSqrt(price)
      // expected 2.718145925979
      assert.ok(result.eq(new BN('1000000000000' + '0'.repeat(PRICE_SCALE - 12))))
    })
  })
  describe('calculate y, liquidity', () => {
    const tokenBN = 6
    const x = new BN(43 * 10 ** (tokenBN - 2)) // 0.43
    ...
    it('in current tick', async () => {
      // rust results:
      const expectedL = new BN('432392997000000')
      ...
      assert.ok(roundUpLiquidity.eq(expectedL))
      assert.ok(roundDownLiquidity.eq(expectedL))
      assert.ok(expectedRoundUpY.eq(roundUpY))
      assert.ok(expectedRoundDownY.eq(roundDownY))
    })
    it('above current tick', async () => {
      // rust results:
      const expectedL = new BN('13548826311623')
  ...
  describe('calculate x, liquidity', () => {
    const tokenBN = 9
    const y = new BN(476 * 10 ** (tokenBN - 1)) // 47.6
    const currentTick = -20000
    const currentSqrtPrice = calculatePriceSqrt(currentTick)

    it('below current tick', async () => {
      // rust results:
      const expectedL = new BN('2789052279103923275')
      ...
      assert.ok(expectedL.eq(roundUpLiquidity))
      assert.ok(expectedL.eq(roundDownLiquidity))
      assert.ok(roundUpX.eq(new BN(0)))
      assert.ok(roundDownX.eq(new BN(0)))
    })
    it('in current tick', async () => {
      // rust results:
      const expectedL = new BN('584945290554346935')
  ...
  describe('calculate liquidity (without specifying on which token)', () => {
    ...
    it('in current tick', async () => {
      // rust results:
      const expectedRoundUpL = new BN('584945290554346935')
      const expectedRoundDownL = new BN('584945290552000000')
      ...
      assert.ok(expectedRoundUpL.eq(roundUpLiquidity))
      assert.ok(expectedRoundDownL.eq(roundDownLiquidity))
      assert.ok(expectedXRoundUp.eq(roundUpX))
      assert.ok(expectedXRoundDown.eq(roundDownX))
    })
    it('above current tick', async () => {
      ...
      const expectedY = new BN(0)
      const expectedL = new BN('13548826311623850')
      ...
      assert.ok(roundingUpLiquidity.eq(expectedL))
      assert.ok(roundingDownLiquidity.eq(expectedL))
  ...
  describe('calculate slippage', () => {
    it('no slippage up', async () => {
      ...
      const limit = limitSqrt.mul(limitSqrt).div(PRICE_DENOMINATOR)
  ...
  describe('calculate x having price and liquidity', () => {
    ...
    it('upperSqrtPrice = 0', async () => {
      ...
      try {
        getX(liquidity, upperSqrtPrice, currentSqrtPrice, lowerSqrtPrice)
      } catch (e: any) {
        assert.isTrue(true)
        return
      }
    ...
    it('getXfromLiquidity', async () => {
      ...
      const x = getXfromLiquidity(liquidity, upperSqrtPrice, lowerSqrtPrice)
      assert.ok(x.eqn(97))
    })
    it('getXfromLiquidity 2', async () => {
      ...
      const x = getXfromLiquidity(new BN(673755091404475), upperSqrtPrice, lowerSqrtPrice)
  ...
  describe('calculate y having liquidity and price', () => {
    ...
    it('lowerSqrtPrice = 0', async () => {
      ...
      try {
        getY(liquidity, upperSqrtPrice, currentSqrtPrice, lowerSqrtPrice)
      } catch (e: any) {
        assert.isTrue(true)
        return
      }
  ...
  describe('test calculateSwapStep', () => {
    it('one token by amount in', async () => {
      const price: BN = PRICE_DENOMINATOR
      const target: BN = sqrt(PRICE_DENOMINATOR.mul(new BN('101')).div(new BN('100')).mul(PRICE_DENOMINATOR))

      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('2000'))
      ...
      assert.ok(result.nextPrice.eq(expectedResult.nextPrice))
    ...
    it('amount out capped at target price', async () => {
      ...
      assert.ok(resultIn.nextPrice.eq(expectedResult.nextPrice))
      ...
      assert.ok(resultOut.nextPrice.eq(expectedResult.nextPrice))
    ...
    it('amount in not capped', async () => {
      const price: BN = PRICE_DENOMINATOR.mul(new BN('101')).div(new BN('100'))
      const target: BN = PRICE_DENOMINATOR.mul(new BN('10'))
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('300000000'))
      ...
      const expectedResult: SwapResult = {
        nextPrice: new BN('1013331333333' + '3'.repeat(PRICE_SCALE - 12)),
    ...
    it('amount out not capped', async () => {
      const price: BN = PRICE_DENOMINATOR.mul(new BN('101'))
      const target: BN = PRICE_DENOMINATOR.mul(new BN('100'))
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('5000000000000'))
      ...
      const expectedResult: SwapResult = {
        nextPrice: new BN('100999999600000' + '0'.repeat(PRICE_SCALE - 12)),
        amountIn: new BN('197'),
  ...
  describe('test getDeltaX', () => {
    it('zero at zero liquidity', async () => {
      const priceA: BN = PRICE_DENOMINATOR.mul(new BN('1'))
      const priceB: BN = PRICE_DENOMINATOR.mul(new BN('1'))
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('0'))
    ...
    it('equal at equal liquidity', async () => {
      const priceA: BN = PRICE_DENOMINATOR.mul(new BN('1'))
      const priceB: BN = PRICE_DENOMINATOR.mul(new BN('2'))
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('2'))
    ...
    it('big numbers', async () => {
      const priceA: BN = new BN('234878324943782000000000000')
      const priceB: BN = new BN('87854456421658000000000000')
      const liquidity: BN = new BN('983983249092')
  ...
  describe('test getDeltaY', () => {
    ...
    it('overflow', async () => {
      const priceA: BN = PRICE_DENOMINATOR
      const priceB: BN = PRICE_DENOMINATOR.muln(2)
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN(2).pow(new BN(64)))
    ...
    it('huge liquidity', async () => {
      const priceA: BN = PRICE_DENOMINATOR
      const priceB: BN = PRICE_DENOMINATOR.addn(1000000)
      const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN(2).pow(new BN(80)))
  ...
  describe('test getNextPriceXUp', () => {
    describe('add', () => {
      it('1', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('1'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('1'))
        const amount: BN = new BN('1')

        const result = getNextPriceXUp(price, liquidity, amount, true)
        const expectedResult: BN = new BN('500000000000' + '0'.repeat(PRICE_SCALE - 12))
      ...
      const result = getNextPriceXUp(price, liquidity, amount, true)
      const expectedResult: BN = new BN('400000000000' + '0'.repeat(PRICE_SCALE - 12))

      assert.ok(result.eq(expectedResult))
      it('3', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('2'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('3'))
        const amount: BN = new BN('5')

        const result = getNextPriceXUp(price, liquidity, amount, true)
        const expectedResult: BN = new BN('461538461538461538461539')

        assert.ok(result.eq(expectedResult))
      })
      it('4', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('24234'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('3000'))
        const amount: BN = new BN('5000')

        const result = getNextPriceXUp(price, liquidity, amount, true)
        const expectedResult: BN = new BN('599985145205615112277488')
    ...
    describe('subtract', () => {
      it('1', async () => {
        ...
        const expectedResult: BN = PRICE_DENOMINATOR.muln(2)

        assert.ok(result.eq(expectedResult))
      })
      it('2', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('100000'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('500000000'))
        const amount: BN = new BN('4000')

        const result = getNextPriceXUp(price, liquidity, amount, false)
        const expectedResult: BN = PRICE_DENOMINATOR.muln(500000)

        assert.ok(result.eq(expectedResult))
      })
      it('3', async () => {
        const price: BN = new BN('3333333333333' + '3'.repeat(PRICE_SCALE - 12))
        const liquidity: BN = new BN('222222222')
        const amount: BN = new BN('37')

        const result = getNextPriceXUp(price, liquidity, amount, false)
        const expectedResult: BN = new BN('7490636713462104974072145')
  ...
  describe('test getNextPriceYDown', () => {
    describe('add', () => {
      it('1', async () => {
        ...
        const expectedResult: BN = new BN('2000000000000' + '0'.repeat(PRICE_SCALE - 12))
        assert.ok(result.eq(expectedResult))
      })
      it('2', async () => {})
      const price: BN = PRICE_DENOMINATOR.mul(new BN('1'))
      ...
      const result = getNextPriceYDown(price, liquidity, amount, true)
      const expectedResult: BN = new BN('2500000000000' + '0'.repeat(PRICE_SCALE - 12))
      assert.ok(result.eq(expectedResult))
      it('3', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('2'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('3'))
        ...
        const result = getNextPriceYDown(price, liquidity, amount, true)
        const expectedResult: BN = new BN('3666666666666' + '6'.repeat(PRICE_SCALE - 12))
      ...
      it('4', async () => {
        ...
        const result = getNextPriceYDown(price, liquidity, amount, true)
        const expectedResult: BN = new BN('24235666666666666' + '6'.repeat(PRICE_SCALE - 12))
    ...
    describe('subtract', () => {
      ...
      it('2', async () => {
        ...
        const result = getNextPriceYDown(price, liquidity, amount, false)
        const expectedResult: BN = new BN('99999999992000000' + '0'.repeat(PRICE_SCALE - 12))

        assert.ok(result.eq(expectedResult))
      })
      it('3', async () => {
        const price: BN = PRICE_DENOMINATOR.mul(new BN('3'))
        const liquidity: BN = LIQUIDITY_DENOMINATOR.mul(new BN('222'))
        const amount: BN = new BN('37')

        const result = getNextPriceYDown(price, liquidity, amount, false)
        const expectedResult: BN = new BN('2833333333333' + '3'.repeat(PRICE_SCALE - 12))
  ...
  describe('test getCloserLimit', () => {
    it('tick limit closer', async () => {
      ...
      const closerLimit: CloserLimit = {
        sqrtPriceLimit: new BN(5).mul(PRICE_DENOMINATOR),
        xToY: true,
        currentTick: 100,
        tickSpacing: 1,
        tickmap: tickmap
      }
      const expected = new BN(5).mul(PRICE_DENOMINATOR)
      const { swapLimit, limitingTick } = getCloserLimit(closerLimit)
      assert.ok(swapLimit.eq(expected))
    ...
    it('trade limit closer', async () => {
      ...
      const closerLimit: CloserLimit = {
        sqrtPriceLimit: new BN(5).mul(new BN(10).pow(new BN(23))),
        xToY: true,
        currentTick: 100,
        tickSpacing: 1,
        tickmap: tickmap
      ...
      const expected = new BN(1).mul(PRICE_DENOMINATOR)
    ...
    it('other direction', async () => {
      ...
      // await tickmap.flip(true, new BN(0), new BN(1))
      const closerLimit: CloserLimit = {
        sqrtPriceLimit: new BN(2).mul(PRICE_DENOMINATOR),
        xToY: false,
        currentTick: -5,
        tickSpacing: 1,
        tickmap: tickmap
    ...
    it('other direction', async () => {
      ...
      const closerLimit: CloserLimit = {
        sqrtPriceLimit: new BN(1).mul(new BN(10).pow(new BN(23))),
        xToY: false,
        currentTick: -100,
        tickSpacing: 10,
        tickmap: tickmap
      ...
      const expected = new BN(1).mul(new BN(10).pow(new BN(23)))
  ...
  describe('test calculateFeeGrowthInside', () => {
    const feeGrowthGlobalX = new BN(15).mul(GROWTH_DENOMINATOR)
    const feeGrowthGlobalY = new BN(15).mul(GROWTH_DENOMINATOR)

    const lowerTick: Tick = {
      pool: Keypair.generate().publicKey,
      index: -2,
      sign: true,
      liquidityChange: new BN(0),
    ...
    it('Subtracts upper tick if below', async () => {
      upperTick.index = 2
      upperTick.feeGrowthOutsideX = new BN(2).mul(GROWTH_DENOMINATOR)
      upperTick.feeGrowthOutsideY = new BN(3).mul(GROWTH_DENOMINATOR)
    ...
    it('Subtracts lower tick if above', async () => {
      upperTick.index = 2
      upperTick.feeGrowthOutsideX = new BN(0)
      upperTick.feeGrowthOutsideY = new BN(0)

      lowerTick.index = -2
      lowerTick.feeGrowthOutsideX = new BN(2).mul(GROWTH_DENOMINATOR)
      lowerTick.feeGrowthOutsideY = new BN(3).mul(GROWTH_DENOMINATOR)
    ...
    it('Test overflow', async () => {
      const feeGrowthGlobalX = new BN(20).mul(GROWTH_DENOMINATOR)
      const feeGrowthGlobalY = new BN(20).mul(GROWTH_DENOMINATOR)

      upperTick.index = -20
      upperTick.feeGrowthOutsideX = new BN(15).mul(GROWTH_DENOMINATOR)
      upperTick.feeGrowthOutsideY = new BN(15).mul(GROWTH_DENOMINATOR)

      lowerTick.index = -10
      lowerTick.feeGrowthOutsideX = new BN(20).mul(GROWTH_DENOMINATOR)
      lowerTick.feeGrowthOutsideY = new BN(20).mul(GROWTH_DENOMINATOR)
  ...
  describe('test calculateTokensOwed', () => {
    ...
    it('zero liquidity fee should not change', async () => {
      const positionData: PositionClaimData = {
        liquidity: new BN(0),
        feeGrowthInsideX: new BN(4).mul(GROWTH_DENOMINATOR),
        feeGrowthInsideY: new BN(4).mul(GROWTH_DENOMINATOR),
        tokensOwedX: new BN(100).mul(DENOMINATOR),
        tokensOwedY: new BN(100).mul(DENOMINATOR)
      }
    ...
    it('fee should change', async () => {
      const positionData: PositionClaimData = {
        liquidity: LIQUIDITY_DENOMINATOR,
        feeGrowthInsideX: new BN(4).mul(GROWTH_DENOMINATOR),
        feeGrowthInsideY: new BN(4).mul(GROWTH_DENOMINATOR),
        tokensOwedX: new BN(100).mul(DENOMINATOR),
        tokensOwedY: new BN(100).mul(DENOMINATOR)
  ...
  describe('test calculateClaimAmount', () => {
    it('Basic claim', async () => {
      const positionData: PositionClaimData = {
        liquidity: new BN(1).mul(LIQUIDITY_DENOMINATOR),
        feeGrowthInsideX: new BN(4).mul(GROWTH_DENOMINATOR),
        feeGrowthInsideY: new BN(4).mul(GROWTH_DENOMINATOR),
        tokensOwedX: new BN(100).mul(DENOMINATOR),
        tokensOwedY: new BN(100).mul(DENOMINATOR)
      ...
      const claim: SimulateClaim = {
        ...
        tickUpper: upperTick,
        tickCurrent: 0,
        feeGrowthGlobalX: new BN(20).mul(GROWTH_DENOMINATOR),
        feeGrowthGlobalY: new BN(20).mul(GROWTH_DENOMINATOR)
      }
    ...
    it('External data', async () => {
      const positionData: PositionClaimData = {
        liquidity: new BN('1479A26FE2A3C0', 'hex'),
        feeGrowthInsideX: new BN('ffffffffffffffffffc8ee8de34d553d', 'hex'),
        feeGrowthInsideY: new BN('ffffffffffffffffffd3fd7d514848b6', 'hex'),
        tokensOwedX: new BN(0),
        tokensOwedY: new BN(0)
      }
      ...
      const lowerTick: Tick = {
        ...
        index: 21109,
        sign: true,
        liquidityChange: new BN('B9C6974437BF7F6B', 'hex'),
        liquidityGross: new BN('B9C6974437BF7F6B', 'hex'),
        sqrtPrice: new BN('029cf3124f61', 'hex'),
        feeGrowthOutsideX: new BN('0c4fee04dd2b3b8c', 'hex'),
        feeGrowthOutsideY: new BN('01a99cb6b2bd6911e7', 'hex'),
        secondsPerLiquidityOutside: new BN(0),
      ...
      const upperTick: Tick = {
        ...
        index: 21129,
        sign: false,
        liquidityChange: new BN('A780979938ACD0B8', 'hex'),
        liquidityGross: new BN('C2B41DADE9987A38', 'hex'),
        sqrtPrice: new BN('029d9e665157', 'hex'),
        feeGrowthOutsideX: new BN('3b9f3a68b9c225', 'hex'),
        feeGrowthOutsideY: new BN('2c0282aeb7b74a', 'hex'),
        secondsPerLiquidityOutside: new BN(0),
  ...
  describe('test simulateSwap', () => {
    it('Swap', async () => {
      ...
      assert.ok(simulationResult.accumulatedAmountIn.eq(new BN(4970)))
      assert.ok(simulationResult.accumulatedAmountOut.eq(new BN(4940)))
      assert.ok(simulationResult.accumulatedFee.eq(new BN(30)))
      assert.ok(simulationResult.amountPerTick[0].eq(new BN(5000)))
      assert.equal(simulationResult.crossedTicks.length, 0)
      assert.ok(simulationResult.priceAfterSwap.eq(new BN('994559659198266500903176')))
      assert.ok(simulationResult.priceImpact.eq(new BN(9861490144)))
      assert.ok(simulationResult.minReceived.eq(new BN(0)))
    })
  })
  describe('test isEnoughAmountToPushPrice', () => {
    const currentPriceSqrt = calculatePriceSqrt(-20)
    const liquidity = new BN('20006000000000000000')
  ...
  describe('test getTickFromPrice', () => {
    ...
    describe('around 0 tick', () => {
      it('get tick at 1', async () => {
        const sqrtPriceBN = new BN(PRICE_DENOMINATOR)
        const tick = priceToTickInRange(sqrtPriceBN, MIN_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, 0)
      })
      it('get tick slightly below 1', async () => {
        const sqrtPriceBN = new BN(PRICE_DENOMINATOR.subn(1))
        const tick = priceToTickInRange(sqrtPriceBN, MIN_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, -1)
      })
      it('get tick slightly above 1', async () => {
        const sqrtPriceBN = new BN(PRICE_DENOMINATOR.addn(1))
        const tick = priceToTickInRange(sqrtPriceBN, MIN_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, 0)
      })
    })
    describe('around 1 tick', () => {
      const sqrtPriceBN = calculatePriceSqrt(1)
      const tickSpacing = 1
      ...
      it('get tick slightly below sqrt(1.0001)', async () => {
        const tick = priceToTickInRange(sqrtPriceBN.subn(1), MIN_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, 0)
      })
      it('get tick slightly above sqrt(1.0001)', async () => {
        const tick = priceToTickInRange(sqrtPriceBN.addn(1), MIN_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, 1)
      })
    })
    describe('around -1 tick', () => {
    ...
    describe('around max - 1 tick', () => {
      const sqrtPriceBN = calculatePriceSqrt(MAX_TICK - 1)
      const tickSpacing = 1
    ...
    describe('around min + 1 tick', () => {
      const sqrtPriceBN = calculatePriceSqrt(MIN_TICK + 1)
      const tickSpacing = 1
      it('get tick at sqrt(1.0001^(-MAX_TICK + 1))', async () => {
        const tick = priceToTickInRange(sqrtPriceBN, MIN_TICK, MAX_TICK, tickSpacing)
    ...
    describe('get tick slightly below and above', () => {
      ...
      const tickSpacing = 1
      it('below', async () => {
        const tick = priceToTickInRange(maxSqrtPrice.subn(1), -MAX_TICK, MAX_TICK + 1, tickSpacing)
        assert.equal(tick, MAX_TICK - 1)
      })
      it('above', async () => {
        const tick = priceToTickInRange(minSqrtPrice.addn(1), -MAX_TICK, MAX_TICK, tickSpacing)
        assert.equal(tick, MIN_TICK)
      })
    })
    describe('around 19_999 tick', () => {
      ...
      const sqrtPriceBN = calculatePriceSqrt(expectedTick)
  ...
  describe('dailyFactorPool tests', () => {
    it('case 1', async () => {
      ...
      const feeTier = { fee: fromFee(new BN(300)), tickSpacing: 30 } // 0.3%
  ...
  describe('dailyFactorReward tests', () => {
    it('case 1', async () => {
      ...
      const result = dailyFactorRewards(reward, tokenXAmount, price, tokenBN, duration)
  ...
  describe('pool APY tests', () => {
    it('case 1', async () => {
      ...
      const current: Tick[] = previous.map(tick => ({
        ...tick,
        liquidityChange: tick.liquidityChange.add(new BN(10000000).mul(LIQUIDITY_DENOMINATOR)),
        feeGrowthOutsideX: tick.feeGrowthOutsideX.add(new BN(10))
      }))
  ...
  describe('position reward APY tests', () => {
    ...
    it('active position 20% of liquidity', async () => {
      const paramsApy: ApyPositionRewardsParams = {
        ...
        tokenBN: 6,
        duration: 10,
        positionLiquidity: poolLiquidity.divn(5),
  ...
  describe('calculate user daily rewards tests', () => {
    ...
    it('active position 20% of liquidity', async () => {
      ...
      const paramsApy: UserDailyRewardsParams = {
        ...
        userLiquidity: poolLiquidity.divn(5),
        duration: 10,
        lowerTickIndex: tickLower,
        upperTickIndex: tickUpper
      }
