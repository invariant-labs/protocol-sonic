...
describe('slippage', () => {
  ...
  before(async () => {
    ...
    expectedPrice = (await market.getPool(pair)).sqrtPrice
  })
  ...
  it('#swap with target just above limit', async () => {
    ...
    const swapVars: Swap = {
      ...
      owner: owner.publicKey,
      amount,
      estimatedPriceAfterSwap: priceLimit,
  ...
  it.only('#swap actual slippage', async () => {
    ...
    const swapVars: Swap = {
      ...
      owner: owner.publicKey,
      amount,
      estimatedPriceAfterSwap: expectedPrice,
    ...
    assert.equal((await market.getPool(pair)).sqrtPrice.toString(), expectedPrice.toString())
