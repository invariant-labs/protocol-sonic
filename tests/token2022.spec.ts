...
import { ClaimFee, CreatePosition, Swap } from '@invariant-labs/sdk/src/market'
...
describe('token2022', () => {
  ...
  it('Check createPoolAndPosition Entrypoint works', async () => {
    const tokens = await Promise.all([
      createToken(connection, wallet, mintAuthority, 6, undefined, true),
      createToken(connection, wallet, mintAuthority)
    ])
    ...
    await market.createPoolAndPosition(
      {
        pair,
        owner: positionOwner.publicKey,
        userTokenX: userTokenXAccount,
