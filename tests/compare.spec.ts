...
import { FeeTier, CreatePosition, Swap } from '@invariant-labs/sdk/lib/market'
...
describe('compare', () => {
  ...
  it('#swap() within a tick', async () => {
    ...
    const liquidityDelta = new BN(2000000).mul(LIQUIDITY_DENOMINATOR)
    ...
    assert.ok(firstPoolData.feeGrowthGlobalX.eq(secondPoolData.feeGrowthGlobalX))
    assert.ok(firstPoolData.feeGrowthGlobalY.eq(secondPoolData.feeGrowthGlobalY))
